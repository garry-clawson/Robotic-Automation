...
The final piece of the development was ensuring that all parts could come together when required. This meant specifying suitable solenoid valves as well as suitable linear guides that would provide a dead stop. These guides would be heavily used so a robust and simple system was needed. The environment within the facility meant that dirt and grime could enter the slideways so simple removal and cleaning was required with a rag at the end of the shift at most. 

To this end Festo were chosen as providers of solenoid values as Festo have a great reputation as well as same or next day delivery with many providers. When specifying a Solenoid it is required to know the diameter pneumatic tube required (I used 6mm low pressure tubing throughout the machine), the trigger voltage. 24V dc was used throughout the machine and also the port options such as 5/2 or 3/2. This just means that you can have 3 ports and 2 positions or 5 ports and 2 positions or have a spring return etc. My application required a 5/2 configuration. 